...
namespace RandomDungeonWithBluePrint
{
    ...
    public static class FieldBuilder
    {
        ...
        private static void MakeBranches(Field field, FieldBluePrint bluePrint)
        {
            ...
            CatLog.Log($"Connection Count: {field.Connections.Count.ToString()}");
        ...
        private static void RemoveAllInvalidConnections(Field field)
        {
            ...
            var relay2RelayConnections = connections.Where(c => !GetSection(c.To).ExistRoom && !GetSection(c.From).ExistRoom).ToList();
