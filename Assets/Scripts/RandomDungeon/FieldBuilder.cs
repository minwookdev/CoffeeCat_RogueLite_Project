...
namespace RandomDungeonWithBluePrint
{
    ...
    public static class FieldBuilder
    {
        ...
        private static void MakeBranches(Field field, FieldBluePrint bluePrint)
        {
            ...
            {
                r2rConnections.Remove(removeTargetConnection);
                connections.Remove(removeTargetConnection);
                CatLog.Log("Remove Relay <-> Relay Connection: From: " + removeTargetConnection.From.ToString() + " -> To: " + removeTargetConnection.To.ToString());
            ...
            // Inner Methods ===========================================================================================
            bool IsExistAnotherConnection(int sectionIndex, Connection self) => field.Connections.Any(c => (c.To == sectionIndex || c.From == sectionIndex) && !(c.To == self.To && c.From == self.From));
            Section GetSection(int index) => field.Sections.SingleOrDefault(s => s.Index == index);
            Connection FindMatchedConnection() => r2rConnections.FirstOrDefault(c => !(IsExistAnotherConnection(c.From, c) && IsExistAnotherConnection(c.To, c)));
            void RemoveInvalidConnections()
            {
                var roomToRelayConnections = connections.Where(c => (!GetSection(c.To).ExistRoom && GetSection(c.From).ExistRoom) || (GetSection(c.To).ExistRoom && !GetSection(c.From).ExistRoom)).ToArray();
                for (int i = 0; i < roomToRelayConnections.Length; i++)
                {
                    // To Section Exist Room
                    if (GetSection(roomToRelayConnections[i].To).ExistRoom)
                    {
                    ...
                    else if (GetSection(roomToRelayConnections[i].From).ExistRoom)
                    {
